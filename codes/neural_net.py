...
import functools
import numpy as np
from utility import alphabetize, abs_mean, plot_decision_boundary
# @implimente:----------------------------------
...
class Neuron(DifferentiableElement):
    ...
    def compute_output(self):
        ...
        # @implimente:----------------------------------
        x = 0.0
        for i , inp in enumerate(self.my_inputs):
            x += inp.output() * self.my_weights[i].get_value()
        return 1 / (1 + np.exp(-x))
        # @end:-----------------------------------------
    ...
    def compute_doutdx(self, elem):
        ...
        # raise NotImplementedError("Implement me!")
        # @implimente:----------------------------------
        gprime = self.output() * (1 - self.output())

        # for direct weight
        if(self.has_weight(elem)):
            for i , w in enumerate(self.my_weights):
                if w == elem:
                    return self.my_inputs[i].output() * gprime

        # for indirect weights
        else:
            s = 0
            for i, w in enumerate(self.my_weights):
                if(self.isa_descendant_weight_of(elem, w)):
                    s += w.get_value() * self.my_inputs[i].dOutdX(elem)
            return s * gprime
        # @end:-----------------------------------------
...
class RegularizedPerformanceElem(PerformanceElem):
    ...
    def regularization(self):
        # self.l*=0.9
        ws = np.array([i.get_value() for i in self.weights])
        ws = np.power(ws , 2)
        return np.sum(ws) * 1/2
    ...
    def output(self):
        ...
        return 1 - (1/2) * ((self.my_desired_val -  self.my_input.output()) ** 2) - self.regularization()*self.l
        # @end:----------------------------------------- 
    ...
    def dOutdX(self, elem):
        ...
        # raise NotImplementedError("Implement me!")
        # @implimente:----------------------------------
        return (self.my_desired_val - self.my_input.output()) * self.my_input.dOutdX(elem) - elem.get_value()*self.l
        # @end:----------------------------------------- 
...
class Network(object):
    ...
    # @implimente:----------------------------------
    def finite_difference(self , epsilon = 10**(-8) , th = 10**(-8)):

        self.clear_cache()
        o0 = self.performance.output()

        for w in self.weights:
            self.clear_cache()
            d  = self.performance.dOutdX(w)
            w0 = w.get_value()

            self.clear_cache()
            w.set_value(w0 + epsilon)
            o1 = self.performance.output()
            w.set_value(w0)

            finite_d = (o1 - o0)/epsilon

            assert(np.abs(d - finite_d) < th)
        print(f"passed finite difference test with acc = {th}")
    # @end:-----------------------------------------
...
def make_neural_net_two_layer():
    ...
    # inputs
    i0 = Input('i0', -1.0)
    i1 = Input('i1', random_weight())
    i2 = Input('i2', random_weight())
...
def make_neural_net_two_moons():
    ...
    # first layer neurons
    alphs = string.ascii_uppercase[0:20]
    for name in ["A" , "B"]:
        for alph in alphs:
            wA  = Weight('w'  + name + alph, random_weight())
            w1A = Weight('w1' + name + alph, random_weight())
            w2A = Weight('w2' + name + alph, random_weight())
            A   = Neuron(name + alph, [i1,i2,i0], [w1A,w2A,wA])
            neurons.append(A)
    assert(len(neurons) == 40)

    # second layer neuron(C)
    wC  = Weight('wC' , random_weight())
    C_weights = []
    for name in ["A" , "B"]:
        for alph in alphs:
            W_C = Weight('w'+name+alph+'C', random_weight())
            C_weights.append(W_C)
    C = Neuron('C', neurons+[i0], C_weights+[wC])

    # performance mesure
    P = RegularizedPerformanceElem(C, 0.0)
    # P = PerformanceElem(C , 0)
...
def train(network,
    ...
    for iteration in tqdm(range(max_iterations) , "train:"):
